...
- **P2 基础检索与 RAG（里程碑）**：NativeRAG(情景层)/LocalSearch(语义层)/GlobalSearch(摘要层)、**混合检索**（稠密 BGE-M3 三输出 + 稀疏 BM25 + 图，RRF 融合）+ **交叉编码器重排**（`bge-reranker-v2-m3`）、答案标注来源文本块、按 AccessContext 过滤可见语料、FastAPI+CLI 暴露 Q&A。**此为"基础功能完善"节点。**
